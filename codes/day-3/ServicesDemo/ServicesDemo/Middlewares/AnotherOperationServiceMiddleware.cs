...
{
    public class AnotherOperationServiceMiddleware
    {
        ...
        public AnotherOperationServiceMiddleware(RequestDelegate next, ILogger<OperationServiceMiddleware> logger, ISingletionOperationService singletionOperationService)
        {
            _next = next;
            _logger = logger;
        ...
        {
            _logger.LogInformation($"Middleware2: Transient: {transientOperationService.OperationId}");
            _logger.LogInformation($"Middleware2: Scoped: {scopedtOperationService.OperationId}");
            _logger.LogInformation($"Middleware2: Singleton: {_singletonService.OperationId}");
            await _next(context);
        }
    }
}